...
if (!class_exists('WPSL_Metaboxes')) {
    ...
    class WPSL_Metaboxes
    {
        ...
        public function meta_box_fields()
        {
            ...
            $meta_fields = array(
                __('Location', 'wpsl') => array(
                    'ta_id' => array(
                        'label' => 'ID кабинета ТА (если есть на селлере)',
                        'required' => false
                    ),
                    'address' => array(
        ...
        function create_meta_fields()
        {
            ...
            <div class="wpsl-store-meta <?php if (floatval($wp_version) < 3.8) {
                echo 'wpsl-pre-38';
            } // Fix CSS issue with < 3.8 versions ?>">
                <?php
                ...
                foreach ($this->meta_box_fields() as $tab => $meta_fields) {
                    ...
                    //print_r(esc_attr( mb_strtolower ( str_replace( ' ', '-', $tab ) ) ) );

                    if ($wpsl_settings['hide_hours'] && $tab == __('Opening Hours', 'wpsl')) {
                        continue;
                    } else {
                        echo '<div class="wpsl-tab wpsl-' . esc_attr( mb_strtolower ( str_replace( ' ', '-', $tab ) ) ) . $active_class . '">';
        ...
        public function meta_field_nav($tab, $active_class)
        {
            ...
            $tab_lower = mb_strtolower(str_replace(' ', '-', $tab));
        ...
        public function text_input($args)
        {
            ...
            <p>
                ...
                <input id="wpsl-<?php echo esc_attr($args['key']); ?>" <?php echo $this->set_required_class($args['data']); ?>
                       type="text" name="wpsl[<?php echo esc_attr($args['key']); ?>]"
        ...
        public function hidden_input($args)
        {
            ...
            <input id="wpsl-<?php echo esc_attr($args['key']); ?>" type="hidden"
                   name="wpsl[<?php echo esc_attr($args['key']); ?>]" value="<?php echo esc_attr($saved_value); ?>"/>
        ...
        public function textarea_input($args)
        {
            ...
            <p>
                <label for="wpsl-<?php echo esc_attr($args['key']); ?>"><?php echo esc_html($args['data']['label']) . ': ' . $this->is_required_field($args['data']); ?></label>
                <textarea
                        id="wpsl-<?php echo esc_attr($args['key']); ?>" <?php echo $this->set_required_class($args['data']); ?>
                        name="wpsl[<?php echo esc_attr($args['key']); ?>]" cols="5"
                        rows="5"><?php echo esc_html($saved_value); ?></textarea>
            </p>
        ...
        public function checkbox_input($args)
        {
            ...
            <p>
                ...
                <input id="wpsl-<?php echo esc_attr($args['key']); ?>" <?php echo $this->set_required_class($args['data']); ?>
                       type="checkbox"
                       name="wpsl[<?php echo esc_attr($args['key']); ?>]" <?php checked($saved_value, true); ?>
                       value="1"/>
            </p>
        ...
        public function dropdown_input($args)
        {
            ...
            if ($args['key'] == 'hours') {
                ...
                <p>
                    <label for="wpsl-<?php echo esc_attr($args['key']); ?>"><?php echo esc_html($args['data']['label']) . ': ' . $this->is_required_field($args['data']); ?></label>
                    <select id="wpsl-<?php echo esc_attr($args['key']); ?>" <?php echo $this->set_required_class($args['data']); ?>
                            name="wpsl[<?php echo esc_attr($args['key']); ?>]" autocomplete="off"/>
                    <?php foreach ($option_list as $key => $option) { ?>
                        <option value="<?php echo esc_attr($key); ?>" <?php if (isset($saved_value)) {
                            selected($saved_value, $key);
            ...
            <table id="wpsl-store-hours" class="<?php echo $hour_class; ?>">
                ...
                foreach ($opening_days as $index => $day) {
                    ...
                    <tr>
                        ...
                        <td id="wpsl-hours-<?php echo esc_attr($index); ?>" class="wpsl-opening-hours"
                            data-day="<?php echo esc_attr($index); ?>">
                            <?php
                            if ($hour_count > 0) {
                                ...
                                while ($i < $hour_count) {
                                    ...
                                    if ((count($hours) == 2) && (!empty($hours[0])) && (!empty($hours[1]))) {
                                        ...
                                        ?>
                                        <div class="wpsl-current-period <?php if ($i > 0) {
                                            echo 'wpsl-multiple-periods';
                                        } ?>">
        ...
        public function map_preview()
        {
            ...
            <p class="wpsl-submit-wrap">
                <a id="wpsl-lookup-location" class="button-primary"
                   href="#wpsl-meta-nav"><?php _e('Preview Location', 'wpsl'); ?></a>
                <span class="wpsl-info"><span
                            class="wpsl-info-text wpsl-hide"><?php echo sprintf(__('The map preview is based on the provided address, city and country details. %s It will ignore any custom latitude or longitude values.', 'wpsl'), '<br><br>'); ?></span></span>
