...
jQuery(document).ready(function ($) {
    ...
    jQuery('#close_register').on('click', function () {
...
// ajaxSetup is global, but we use it to ensure JSON is valid once returned.
    ...
    APP = window.POS_APP = {
            ...
            APP.db.get('orders', order_id).always(function (order) {
                ...
                if (typeof order != 'undefined') {
                    ...
                    if (order.status != 'pending'){
                        ...
                        $("#wc-pos-register-buttons .tbr").removeClass("hide_btn");
                    }
                    ...
                    $.each(order.line_items, function (index, item) {
                        ...
                        if (pos_custom_product.id == item.product_id || item.product_id == null) {
                            ...
                            APP.addToCart(item.product_id, quantity, variation_id, variation, 0, item.id, item);
                        }
        ...
        addToCart: function (product_id, quantity, variation_id, variation, cart_item_data, item_id, item) {
        ...
        insertSearchListItem: function (obj) {
            ...
            APP.tmp.products[obj.id] = {id: val, text: t};
            ...
            if (obj.variations.length > 0) {
                for (j = 0; j < obj.variations.length; j++) {
                    ...
                    APP.tmp.products[v.id] = {id: val, text: name};
            ...
            if (pos_grid.second_column_layout == 'product_grids') {
                ...
                if (pos_grid.grid_id == 'categories') {
                    $.each(pos_grid.categories, function (i, cat) {
                        var $li = $('<li id="category_' + cat.term_id + '" class="title_category open_category category_cycle" data-catid="' + cat.term_id + '"><span></span></li>');
        ...
        processPayment: function (cart, paid) {
            var v = APP.makeid();
        ...
        ready: function () {
            ...
            $('#check_save_customer').on('click', function () {
                ...
                $.each(wc_pos_params.a_billing_fields, function (index, key) {
                    ...
                    if (key == 'number_simcard') {
                        var a_el = $('#pos_billing_details #number_simcard option:selected');
                        var _val = [];
                        a_el.filter(':selected').each(function (index, el) {
                            _val.push($(el).val());
                        });
            ...
            $('#save_customer').on('click', function () {
                ...
                if (err == 0) {
                    ...
                    var customer_upload_field = '';
                    if  (typeof CUSTOMER.additional_fields['uploaded_files'] !== 'undefined'){
                        customer_upload_field = CUSTOMER.additional_fields['uploaded_files'];
                        //console.log(CUSTOMER.additional_fields['uploaded_files']);
                    }
                    if (new_customer) {
                        CUSTOMER.reset();
                    }
            ...
            $('#save_order_discount').click(function (e) {
            ...
            $('#retrieve_sales').click(function () {
                var reg_id = $('#bulk-action-retrieve_sales option').val();
                var register_name = $('#bulk-action-retrieve_sales option:selected').data('name');
                retrieve_sales(reg_id, register_name);
                return false;
            });
            ...
            $('#modal-retrieve_sales').on('keypress', '#orders-search-input', function (e) {
                if (e.which == 13) {
                    var register_id = $('#bulk-action-retrieve_sales option').val();
    ...
    resizeCart = function () {
        ...
        if ($('#order_items_list-wrapper').length) {
            $('#order_items_list-wrapper').height(h);
        } else {
            var wrap = $('<div id="order_items_list-wrapper"></div>').height(h);
