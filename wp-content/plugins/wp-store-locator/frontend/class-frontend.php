    ...
    class WPSL_Frontend
    {
        ...
        public function create_opening_hours_tabel($hours, $hide_closed)
        {
            ...
            if ($this->not_always_closed($hours)) {
                ...
                $hour_table .= '<span style="font-weight: 600">Режим работы:</span>';
        ...
        public function cpt_template($content)
        {
            ...
            if (isset($post->post_type) && $post->post_type == 'wpsl_stores' && is_single() && in_the_loop()) {
                ...
                $content .= '<div class="wpsl_page_ta_simcards">';
        ...
        public function show_store_simcards($atts)
        {
            ...
            if ($atts['ta_id'] && $store_address = get_post_meta($atts['id'], 'wpsl_ta_id', true)) {
                ...
                foreach ($array_of_simcard as $key => $oper) {
                    ...
                    $content .= '<h4 id="operator_label">' . $simcard . '</h4>';
                    if ($key == 'orange') {
                        $combo = 0;
                        $nano = 0;
                        foreach ($oper as $num) {
                        ...
                        $content .= '<div id="format_grid">';
                        $content .= '<div id="format_grid_name">';
                        $content .= '<p class="format_label">Комбо</p>';
                        $content .= '<p class="format_description">(стандарт+микро)</p>';
                        $content .= '</div>';
                        $content .= '<div id="format_grid_count">';
                        ...
                        $content .= '<div id="format_grid">';
                        $content .= '<div id="format_grid_name">';
                        $content .= '<p class="format_label">3 в 1</p>';
                        $content .= '<p class="format_description">(стандарт+микро+нано)</p>';
                        $content .= '</div>';
                        $content .= '<div id="format_grid_count">';
                        $content .= '<p>' . $nano . ' шт.</p>';
                        $content .= '</div>';
...
//            }
            ...
            if ($atts['address2'] && $store_address2 = get_post_meta($atts['id'], 'wpsl_address2', true)) {
                $content .= '<span><span style="font-weight: 600">Дополнительный адрес: </span>' . esc_html($store_address2) . '</span><br/>';
            }
