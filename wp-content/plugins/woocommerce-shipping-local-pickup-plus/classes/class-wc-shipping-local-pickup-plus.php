...
//        print_r($state);
        ...
        echo '<p>Выберите пункт самовывоза из выпадающего списка</p>';
        ...
        if (1 === count($this->pickup_locations)) {
            ...
            if ('select' === $this->get_checkout_pickup_location_styling()) {
                ...
                foreach ($this->pickup_locations as $location) {
                    ...
                    echo '<input type="radio" name="pickup_location[' . $package_index . ']" id="pickup_location_' . $package_index . '_' . $location['id'] . '" value="' . esc_attr($location['id']) . '" class="pickup_location" ' . checked($location['id'], $chosen_pickup_location_id, false) . ' />';
                    echo '<label for="pickup_location_' . $package_index . '_' . $location['id'] . '">' . $this->get_formatted_address_helper($location, true, true) . '</label>';
                    echo '</li>';
                }
                echo '</ul>';
            }
        ...
        foreach (WC()->cart->get_cart() as $cart_item) {
            ...
            switch ($product_id) {
                case 23065:
                    if (!array_key_exists(23065, $products_in_cart))
                        $products_in_cart[23065] = 'orange';
                    break;
                case 23047:
                    if (!array_key_exists(23047, $products_in_cart))
                        $products_in_cart[23047] = 'vodafone';
                    break;
                case 23090:
                    if (!array_key_exists(23090, $products_in_cart))
                        $products_in_cart[23090] = 'ortel';
                    break;
                case 23042:
                    if (!array_key_exists(23042, $products_in_cart))
                        $products_in_cart[23042] = 'globalsim--classic';
                    break;
                case 23085:
                    if (!array_key_exists(23085, $products_in_cart))
                        $products_in_cart[23085] = 'globalsim--gsim_internet';
                    break;
                case 23031: //TravelChat
                    if (!array_key_exists(23031, $products_in_cart))
                        $products_in_cart[23031] = 'globalsim--gsim_internet';
                    break;
                case 23026:
                    if (!array_key_exists(23026, $products_in_cart))
                        $products_in_cart[23026] = 'globalsim--tariff_usa';
                    break;
                case 23037:
                    if (!array_key_exists(23037, $products_in_cart))
                        $products_in_cart[23037] = 'globalsim--europasim';
                    break;
            }
        }
        ...
        if ($chosen_pickup_location && isset($chosen_pickup_location['taid']) && $chosen_pickup_location['taid']) {
            ...
            echo '<p style="margin-bottom: 0;color:#fff;font-weight:600;font-size:17px;text-align:center;padding:12px 0;">Наличие сим-карт в пункте выдачи</p>';

            $content = '<div class="wpsl-page-ta-simcards-grid" style="background:#656565;padding:20px 0 20px 0;">';

            foreach ($array_of_simcard as $key => $oper) {

                if (!in_array($key, $products_in_cart)) continue;
            ...
            echo '<div style="text-align: left;padding: 14px;color: #fff;font-weight: 400;">';
    ...
    public function order_pickup_location($order)
    {
        ...
        foreach ($pickup_locations as $pickup_location) {
            ...
            if (isset($pickup_location['taid']) && $pickup_location['taid']) {
                echo '<div>' . $pickup_location['taid'] . '</div>';
            }
        }
    ...
    public function email_pickup_location_plain($order)
    {
        ...
        foreach ($pickup_locations as $pickup_location) {
            ...
            if (isset($pickup_location['taid']) && $pickup_location['taid']) {
                echo wp_kses($pickup_location['taid']) . "\n";
            }
        }
    }
    ...
    private function get_order_pickup_locations($order)
    {
        ...
        foreach ($order->get_shipping_methods() as $shipping_item) {
            ...
            if ($this->id == $shipping_item['method_id'] && isset($shipping_item['pickup_location'])) {
                ...
                $location['taid'] = isset($global_location['taid']) ? $global_location['taid'] : '';
    ...
    private function get_pickup_address_fields($country)
    {
        ...
        unset($address_fields['shipping_first_name'], $address_fields['shipping_last_name'], $address_fields['shipping_country'], $address_fields['shipping_postcode'], $address_fields['shipping_address_2']);
        $pickup_fields = array();
