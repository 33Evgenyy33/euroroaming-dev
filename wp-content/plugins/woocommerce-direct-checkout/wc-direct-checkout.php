...
if(!class_exists('WooCommerce_Direct_Checkout')){
	class WooCommerce_Direct_Checkout{
		...
		function custom_add_to_cart_redirect() {
			...
			if($has_virtual_products != true) return;
		...
		function direct_checkout_continue_button() {
			...
			$get_virtual = get_post_meta($product->id,'_virtual',true);


			if($direct_checkout_continue_enabled == "1" && $get_virtual == "yes"){
			...
			if( $direct_checkout_continue_enabled && $get_virtual == "yes"){
		...
		function custom_cart_button_text() {
			...
			//$prod_cat = get_term_by( 'id', $product->id, 'product_cat', 'ARRAY_A' );
			//var_dump('<div class="categgg">'.$prod_cat['name']);
			...
			if($get_virtual == "yes") $direct_checkout_cart_button_text = "Пополнить баланс";
			
			if ($get_virtual == "yes" && $product->id == 23575){
				$direct_checkout_cart_button_text = "Активировать сим-карту";
			}
			if ($get_virtual == "yes" && $product->id == 25841){
				$direct_checkout_cart_button_text = "Перейти к восстановлению";
			}

			
			//$categ = $product->get_categories( );
			
			if($get_virtual == "yes" && $product->id == 22325){
				$direct_checkout_cart_button_text = "Подключить";
			}
