...
class US_Layout {
	...
	protected function __construct() {
		...
		if ( is_singular( array_merge( array( 'page', 'product' ), $supported_custom_post_types ) ) OR ( is_404() AND $postID != NULL ) ) {
			...
			if ( usof_meta( 'us_titlebar_content', array(), $postID ) == 'hide' ) {
				$this->titlebar = 'none';
			} elseif ( in_array( usof_meta( 'us_titlebar_content', array(), $postID ), array( 'caption', 'all', ) ) ) {
		...
		if ( is_singular(
			...
			if ( usof_meta( 'us_sidebar', array(), $postID ) != '' ) {
				$this->sidebar_pos = usof_meta( 'us_sidebar', array(), $postID );
			}

			global $us_iframe;
			if ( ( isset( $us_iframe ) AND $us_iframe ) OR usof_meta( 'us_header_remove', array(), $postID ) ) {
				$this->header_show = 'never';
				$this->header_orientation = 'none';
			} elseif ( usof_meta( 'us_header_sticky_pos', array(), $postID ) != '' AND $this->header_orientation == 'hor' AND $this->sidebar_pos == 'none' AND $this->titlebar == 'none' ) {
