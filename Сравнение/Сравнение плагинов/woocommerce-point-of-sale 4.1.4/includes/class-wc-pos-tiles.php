      ...
      $data['tile_style']       = $data['style'];
    	...
    	<form id="edit_wc_pos_outlets" class="validate" action="" method="post">
    		...
    		<input type="hidden" value="wc_pos_edit_update_tiles" name="action">
				...
				<table class="form-table">
					<tbody>
						<tr class="form-field form-required">
							...
							<input type="hidden" id="product_id" name="product_id" class="ajax_chosen_input_products" style="width: 400px;" data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-selected="<?php echo $selected; ?>" value="<?php echo $value; ?>" />
								...
								<ul class="wc-radios">
									<li>
										<label for="tile_style_image">
											<input type="radio" name="tile_style" class="tile_style" id="tile_style_image" value="image" <?php echo (isset($data['tile_style']) && $data['tile_style'] == 'image') ? 'checked="checked"' : ''; ?>/>
											<?php _e( 'Image', 'wc_point_of_sale' ); ?>
										</label>										
									</li>
									<li>
										<label for="tile_style_colour">
											<input type="radio" name="tile_style" class="tile_style" id="tile_style_colour" value="colour"  <?php echo (isset($data['tile_style']) && $data['tile_style'] == 'colour') ? 'checked="checked"' : ''; ?>/>
						...
						<tr class="form-field form-required tile_style_bg_row">
							...
							<td>
								...
								<?php if($data['background_color']){ ?>
									<input type="text" name="background_color" id="background_color" value="#<?php echo $data['background_color']; ?>" />
								<?php }else{ ?>
									<input type="text" name="background_color" id="background_color" value="#<?php echo esc_attr( get_background_color() ); ?>" />
								<?php } ?>
								<p class="description"><?php _e( 'Select the product which this tile will represent.', 'wc_point_of_sale' ); ?></p>
							</td>
						</tr>
						<tr class="form-field form-required tile_style_bg_row">
							...
							<td>
								<?php if($data['text-color']){ ?>
									<input type="text" name="text-color" id="text-color" value="#<?php echo $data['text-color']; ?>" />
								<?php }else{ ?>
									<input type="text" name="text-color" id="text-color" value="#<?php echo esc_attr( get_background_color() ); ?>" />
								<?php } ?>
						...
						<tr class="form-field form-required" >
							...
							<td>
									<div class="preview_default" id="custom-background-image1" style="background: #<?php echo $data['background_color']; ?>;" data-shop_thumbnail= "<?php echo $image; ?>">
										<span class="style1" id="custom-background-tiles-color" style="color: #<?php echo $data['text-color']; ?>;">
											your text here
										</span>
									</div>
							</td>
				...
				<p class="submit">
					<input type="submit" name="add_new_tiles" id="submit" class="button button-primary" value="<?php _e( 'Update', 'wc_point_of_sale' ); ?>"></p>
    	</form>
  	</div>
		<?php
	}
		    ...
		    <div id="col-container">
              ...
              <h2><?php _e('Grid Preview ', 'wc_point_of_sale'); ?></h2>
            	<div class="postbox wc-pos-preview-register-grids" id="wc-pos-register-grids">
							  <h3 class="hndle"><span><?php _e( ucfirst($grids_single_record[0]->name).' Layout', 'wc_point_of_sale' ) ?></span></h3>
                          ...
                          if ( $titles ) :
                              ...
                              foreach ($titles as $title) :
                                  ...
                                  if($title->style == 'image'){
                                    ...
                                    ?>
                                    <td id="title_<?php echo $title->ID ?>" style="background: url('<?php echo $image; ?>') top no-repeat; background-size: auto 60px; background-color: #fff; vertical-align: bottom;" class="title_product">
                                      <a style="color: #222222; margin-bottom: 5px; display: block; font-weight: normal; font-size: 12px;" data-id="<?php echo $product_id; ?>"><?php echo get_the_title($title->product_id) ;?></a>
                                    </td>
                                    <?php
                                  }else{ ?>
                                    <td id="title_<?php echo $title->ID ?>" style="background: #<?php echo $title->background;?>; " class="title_product">
                                      <a style="color: #<?php echo $title->colour;?>;" data-id="<?php echo $product_id; ?>"><?php echo get_the_title($title->product_id) ;?></a>
                                    </td>
                                    <?php 
                                  }
              ...
              <div class="previous-next-toggles">
		    	...
		    	?>
		    				...
		    				<h3><?php _e( 'Add New '.ucfirst($grids_single_record[0]->name).' Tile', 'wc_point_of_sale' ) ?></h3>
									...
									<ul class="wc-radios">
										<li>
											<label for="products_opt">
												<input type="radio" id="products_opt" name="products_or_cat" value="product" checked />
												<?php _e('Product', 'wc_point_of_sale'); ?>
											</label>
										</li>
										<li>
											<label for="category_opt">
												<input type="radio" id="category_opt" name="products_or_cat" value="category" />
								...
								<div id="category_opt_wrap">
									<div class="form-field">
										...
										?>
										<select id="category_id" class="category_chosen" style="width: 294px;" name="category_id">
								...
								<div id="products_opt_wrap">
									<div class="form-field">
										<label for="product_id"><?php _e( 'Product', 'wc_point_of_sale' ); ?></label>
                                        <select id="product_id" name="product_id" class="wc-product-search ajax_chosen_input_products" style="width: 400px;" data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce' ); ?>" ></select>
										<p class="description"><?php _e( 'Select the product which this tile will represent.', 'wc_point_of_sale' ); ?></p>
									</div>
									<div class="form-field">
										<label for="tile_style"><?php _e( 'Style', 'wc_point_of_sale' ); ?></label>
										<ul class="wc-radios">
											<li>
												<label for="tile_style_image">
													<input type="radio" name="tile_style" class="tile_style" id="tile_style_image" value="image" <?php echo ($tile_style == 'image') ? 'checked="checked"' : ''; ?>/>
													<?php _e( 'Image', 'wc_point_of_sale' ); ?>
												</label>										
											</li>
											<li>
												<label for="tile_style_colour">
													<input type="radio" name="tile_style" class="tile_style" id="tile_style_colour" value="colour"  <?php echo ($tile_style == 'colour') ? 'checked="checked"' : ''; ?>/>
													<?php _e( 'Colour', 'wc_point_of_sale' ); ?>
												</label>										
											</li>
										</ul>
									...
									<div class="form-field tile_style_bg_row">
										<label for="background_color"><?php _e( 'Background Color', 'wc_point_of_sale' ); ?></label>
											<input type="text" name="background_color" id="background_color" value="#<?php echo $background_color; ?>" />
										<p class="description"><?php _e( 'Select the background colour for this tile.', 'wc_point_of_sale' ); ?></p>
									</div>
									<div class="form-field tile_style_bg_row">
										<label for="text-color"><?php _e( 'Text Color', 'wc_point_of_sale' ); ?></label>
										<input type="text" name="text-color" id="text-color" value="#<?php echo $text_color; ?>" />
										<p class="description"><?php _e( 'Select the text colour for this tile.', 'wc_point_of_sale' ); ?></p>
									</div>
									<div class="form-field dafault_selection" style="display: none;" >
										<label for="dafault_selection"><?php _e( 'Dafault Selection', 'wc_point_of_sale' ); ?></label>
										<select name="dafault_selection" id="dafault_selection" style="width: 400px;" ></select>
										<p class="description"><?php _e( 'Selection the Default Section for this using variations.', 'wc_point_of_sale' ); ?></p>
									</div>

									<div class="form-field">
										<label for="preview"><?php _e( 'Preview', 'wc_point_of_sale' ); ?></label>
										<div class="preview_default" id="custom-background-image1" style="background: #<?php echo  $background_color; ?>;" data-shop_thumbnail= "<?php echo $image; ?>">
											<span class="style1" id="custom-background-tiles-color"  style="color: #<?php echo  $text_color; ?>;">
												your text here
											</span>
										</div>
									</div>
					...
					<p class="submit">
					<input type="submit" name="add_new_tiles" id="submit" class="button button-primary" value="<?php _e( 'Add New Tile', 'wc_point_of_sale' ); ?>"></p>
